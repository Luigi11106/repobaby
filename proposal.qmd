...
Pushed back: (2.  To what extent do the objective economic situation (net household income) and the subjective expectation regarding one's own future financial situation influence support for state income redistribution in Germany during the pandemic year of 2021?)
...
library(here)
library(haven)
allbus <- read_dta(here("data", "raw", "allbus_mini.dta"))
```
...
allbus %>%
  ...
  #labels
  labs(title = "Abstimmungsabsicht in Deutschland \n nach subjektiv wahrgenommener Wirtschaftslage",
         x = "Subjektive wirtschafltiche Lage in DE heute",
         y = "Prozent",
         fill = "Abstimmungsabsicht")+
  scale_fill_manual(values = c("red2", "cyan3"), labels = c("Nicht abstimmen", "Abstimmen"))+
...
allbus %>%
  group_by(pv01) %>%
  summarise("Anzahl Personen" = n()) |> 
  rename("Wahlabsicht BTW (pv01)" = pv01)
...
allbus %>%
  ...
  geom_bar(position = "fill") +
  labs(x = "Wirtschaftslage \n (1 = sehr gut, 5 = sehr schlecht)",
       y = "Prozent",
       fill = "Region",
       title = "Einordnung der Wirtschaftslage in Deutschland")+
  scale_fill_manual(labels = c("west", "ost"), values = c("red2", "cyan3"))
```
